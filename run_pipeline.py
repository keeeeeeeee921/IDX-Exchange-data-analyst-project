...
Stages: monthly aggregation → EDA/FRED enrichment → cleaning → feature
engineering → IQR outlier flagging → Tableau prep → EDA report.
...
Run with the project venv so every stage gets the right deps:
...
STAGES = [
    ...
    "eda_report.py",                       # statewide EDA report
]
